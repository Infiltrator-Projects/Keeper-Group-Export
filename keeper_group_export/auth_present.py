"""Login and authentication-choice presentation."""
...
class AuthPresentationMixin:
    def show_login_screen(self):
        ...
        # Reserve enough vertical space for wrapped authentication errors.
        ...
        self.login_hint = tk.Label(
            card,
            text=(
                "Preparing Keeper in the background…"
        ...
        pass_entry.bind("<Return>", lambda _event: self._start_connect_from_login())
        user_entry.bind("<Return>", lambda _event: pass_entry.focus_set())
        if self.last_user:
    ...
    def _choose_from_list(self, title, prompt, choices):
        ...
        listbox = tk.Listbox(
            win,
            height=min(max(len(choices), 2), 8),
            bg=C_INPUT,
            fg=C_TEXT,
        ...
        listbox.bind("<Return>", lambda _event: accept())
