"""Keeper Group Export ui layout module."""
...
class UILayoutMixin:
    def _build_ui(self):
        ...
        about_nav = tk.Button(
            ...
            activebackground="#242424", activeforeground=C_TEXT,
            relief="flat", bd=0,
            font=("Segoe UI", 9),
            anchor="w", padx=11, pady=10
        )
        about_nav.pack(fill="x", padx=10, pady=(4, 0))

        sidebar_spacer = tk.Frame(sidebar, bg=C_SIDEBAR)
        ...
        tk.Label(
            footer, text=f"Version {APP_VERSION}",
            bg=C_SIDEBAR, fg="#727272",
            font=("Segoe UI", 8), anchor="w"
        ).pack(anchor="w")
        tk.Label(
            footer, text="Shannon Smith • Carlo Cunanan",
            bg=C_SIDEBAR, fg="#727272",
        ...
        tk.Label(
            title_block, text="Keeper Group Export",
            bg=C_BG, fg=C_TEXT,
        ...
        status_card = tk.Frame(
            content, bg=C_CARD,
            highlightbackground="#353535", highlightthickness=1
        )
        ...
        self.status_dot = tk.Canvas(
            status_title, width=12, height=12,
            bg=C_CARD, highlightthickness=0
        )
        self.status_dot.create_oval(
            2, 2, 10, 10,
            fill=C_WARNING, outline=C_WARNING, tags="dot"
        )
        self.status_dot.pack(side="left", padx=(0, 8))

        tk.Label(
            status_title, text="Vault connection",
            bg=C_CARD, fg=C_TEXT,
            font=("Segoe UI Semibold", 10)
        ).pack(side="left")

        self.conn_status = tk.Label(
            status_left, text="Preparing Keeper library…",
        ...
        tk.Label(
            preview_title, text="Credential preview",
            bg=C_CARD, fg=C_TEXT,
            font=("Segoe UI Semibold", 10)
        ).pack(anchor="w")
        ...
        # Search affects only the preview. The underlying ``rows`` collection
        # remains the complete selected group and export remains unfiltered.
        self.search_var.trace_add("write", lambda *_: self._populate_preview())

        # Desktop-style shortcuts make the utility feel like a normal Windows
        # application without introducing hidden functionality.
        self.bind_all("<Control-f>", lambda event: self._focus_search())
        self.bind_all("<Control-e>", lambda event: self._shortcut_export())
        self.bind_all("<F5>", lambda event: self._shortcut_refresh())
